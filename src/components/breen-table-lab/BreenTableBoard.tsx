      ...
      <div className="btl-board__rows">
        {CANONICAL_BREEN_GRID_RTL.map((row, rowIndex) => (
          <div key={`row-${rowIndex + 1}`} className="btl-board__row">
            {showRowLabels ? (
              <div className="btl-board__row-label">
                <span>שורה {rowIndex + 1}</span>
              </div>
            ) : null}
                ...
                const body = (
                  <>
                    <div className="btl-board__cell-top">
                      {state.badge ? <span className="btl-board__badge">{state.badge}</span> : <span className="btl-board__family">{cell.familyHe}</span>}
                    </div>
