...
.btl-app{direction:rtl;font-family:"Assistant","Heebo","Noto Sans Hebrew","Segoe UI",sans-serif;color:#14263d;max-width:1240px;margin:0 auto}
.btl-app *{box-sizing:border-box}
.btl-shell{display:grid;gap:16px;background:
  radial-gradient(circle at top right,rgba(79,70,229,.12),transparent 28%),
  radial-gradient(circle at left top,rgba(14,165,233,.11),transparent 30%),
  linear-gradient(180deg,#f7f9fc 0%,#fcfdff 100%);
  border:1px solid #dde6f2;border-radius:32px;padding:18px;box-shadow:0 26px 64px rgba(15,23,42,.08)}
.btl-kicker{font-size:.8rem;font-weight:900;letter-spacing:.04em;color:#31567f}
.btl-title{font-size:clamp(1.5rem,2.1vw,2.3rem);line-height:1.08;font-weight:900;margin:0}
.btl-subtitle,.btl-body,.btl-copy{color:#53667d;line-height:1.68}
.btl-btn-row,.btl-pill-row,.btl-chip-row,.btl-stat-row,.btl-header-actions,.btl-option-row,.btl-tray,.btl-inline-actions{display:flex;flex-wrap:wrap;gap:10px;align-items:center}
...
.btl-play-shell{display:grid;gap:14px}
.btl-topbar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:start;background:rgba(255,255,255,.96);border:1px solid #dde6f2;border-radius:24px;padding:16px;box-shadow:0 16px 36px rgba(15,23,42,.05)}
.btl-topbar h2{margin:0;font-size:1.12rem}
.btl-topbar p{margin:0}
.btl-status-grid{display:grid;gap:10px;justify-items:end}
...
.btl-play-main{display:grid;gap:14px}
.btl-phase-strip{display:flex;flex-wrap:wrap;gap:10px}
.btl-phase{display:inline-flex;align-items:center;gap:8px;border:1px solid #dce5f2;background:rgba(255,255,255,.88);padding:8px 12px;border-radius:999px;font-weight:800;color:#54657c}
.btl-phase[data-active="1"]{background:#eef4ff;border-color:#bfd1fb;color:#1e3a8a}
.btl-workbench{display:grid;gap:14px}
.btl-main-surface{display:grid;gap:14px;background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(248,250,255,.98));border:1px solid #dde6f2;border-radius:28px;padding:18px;box-shadow:0 18px 40px rgba(15,23,42,.06)}
.btl-prompt{display:grid;gap:10px;border:1px solid #dbe5f1;border-radius:22px;background:#fbfcff;padding:14px}
.btl-prompt strong{font-size:1rem}
.btl-prompt p{margin:0;color:#5a6d84;line-height:1.58}
.btl-tray{margin-top:4px}
.btl-tray-item{border:1px solid #dce5f2;background:#fff;border-radius:999px;padding:9px 13px;font:inherit;font-weight:800;cursor:pointer;transition:.18s ease}
.btl-tray-item:hover:not(:disabled){transform:translateY(-1px);border-color:#a1bdf6}
.btl-tray-item:disabled{opacity:.6;cursor:not-allowed}
.btl-tray-item[data-selected="1"]{background:#eef4ff;border-color:#1d4ed8;color:#1d4ed8}
.btl-tray-item[data-placed="1"]{background:#f5f7fb;color:#94a3b8}
.btl-helper-line{font-size:.88rem;color:#4c647f}
.btl-feedback-bar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;border:1px solid #dbe5f1;background:rgba(255,255,255,.95);border-radius:24px;padding:14px 16px;box-shadow:0 12px 28px rgba(15,23,42,.05)}
...
.btl-choice-inline-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}
...
.btl-board{display:grid;gap:14px}
...
.btl-board__row{display:grid;grid-template-columns:110px minmax(0,1fr);gap:10px;align-items:stretch}
.btl-board__row-label{display:flex;align-items:center;justify-content:center;border-radius:18px;background:#f5f7fb;border:1px solid #dde6f2;color:#61738a;font-weight:800;font-size:.82rem;padding:8px;text-align:center}
.btl-board__grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
.btl-board__cell{min-height:96px;border-radius:20px;border:1px solid #dbe5f1;background:linear-gradient(180deg,#ffffff 0%,#fbfdff 100%);padding:12px;display:grid;align-content:space-between;gap:10px;text-align:right;color:#13253f;transition:.18s ease;box-shadow:0 10px 24px rgba(15,23,42,.04)}
.btl-board__cell[data-family="distortion"]{box-shadow:inset 0 0 0 1px rgba(99,102,241,.04),0 10px 24px rgba(15,23,42,.04)}
.btl-board__cell[data-family="generalization"]{box-shadow:inset 0 0 0 1px rgba(13,148,136,.04),0 10px 24px rgba(15,23,42,.04)}
.btl-board__cell[data-family="deletion"]{box-shadow:inset 0 0 0 1px rgba(34,197,94,.04),0 10px 24px rgba(15,23,42,.04)}
.btl-board__cell[data-family="extra"]{box-shadow:inset 0 0 0 1px rgba(245,158,11,.04),0 10px 24px rgba(15,23,42,.04)}
...
.btl-board__cell.is-active{box-shadow:0 0 0 2px rgba(29,78,216,.08),0 10px 24px rgba(15,23,42,.04)}
.btl-board__cell.is-dimmed{opacity:.55}
.btl-board__cell-top{display:flex;justify-content:space-between;gap:8px;align-items:center}
.btl-board__family,.btl-board__badge{font-size:.72rem;font-weight:800;color:#60738b}
.btl-board__cell-main{display:grid;gap:6px}
.btl-board__cell-main strong{font-size:.92rem;line-height:1.34}
.btl-board__cell-main small{color:#66778d;line-height:1.35}
.btl-board__placeholder{font-size:1.3rem;line-height:1;color:#b7c3d1}
.btl-board__cell.is-ghost{background:linear-gradient(180deg,#ffffff 0%,#fafcff 100%)}
...
.btl-board__cell.is-placed{background:#fffaf0;border-color:#fcd34d}
.btl-board__cell.is-missing{background:#fffaf0;border-style:dashed;border-color:#f3c76b}
.btl-board__cell.is-prompt{background:#f8fafc;border-color:#cbd5e1}
...
@media (max-width:1100px){
  .btl-hero,.btl-mini-demo,.btl-setup-grid,.btl-results-grid{grid-template-columns:1fr}
  .btl-tag-grid{grid-template-columns:repeat(2,minmax(0,1fr))}
}

@media (max-width:780px){
  .btl-shell{padding:12px;border-radius:24px}
  .btl-mode-grid,.btl-option-grid,.btl-choice-inline-grid,.btl-metric-grid{grid-template-columns:1fr}
  .btl-topbar,.btl-feedback-bar{grid-template-columns:1fr}
  ...
  .btl-board__row-label{min-height:44px}
}
...
@media (max-width:560px){
  ...
  .btl-tray-item,.btl-btn{width:100%;justify-content:center}
  .btl-btn-row,.btl-header-actions,.btl-tray,.btl-inline-actions{display:grid;grid-template-columns:1fr}
