...
const SCREEN_READ_GUIDES = Object.freeze({
    ...
    'practice-wizard': Object.freeze({
        ...
        goal: 'לנסח משפט גשר שיושב גם בחוויה וגם במציאות, ורק אחר כך לבדוק מה התבהר על המשפט המקורי.',
...
const THERAPEUTIC_DEMO_BY_SCREEN = Object.freeze({
    ...
    'practice-wizard': Object.freeze({
        ...
        outcomes: Object.freeze([
            ...
            'לבדוק מה התבהר על המשפט רק אחרי שיש תחושת הלימה ולא במקום הלימה.'
        ])
    }),
    'practice-verb-unzip': Object.freeze({
        turns: Object.freeze([
...
function buildScreenReadGuide(screenId) {
    ...
    const philosopherIconSrc = resolveVersionedAssetPath('assets/svg/props/philosopher-guide.svg');
    const philosopherTeaser = screenId === 'categories'
        ? 'כאן נפתחת מפת ברין המלאה: לא רק שמות תבניות, אלא איך כל תבנית חושפת מידע שהיה חסר.'
        : 'לחיצה פותחת את מנגנון העומק: מאיפה נולד הדפוס, מה המחיר שלו, ומה השאלה שמחזירה תנועה.';
    ...
    philosopherToggle.innerHTML = `
        <summary class="screen-read-guide-philosopher-summary" aria-label="פילוסוף מסך - פתיחת ההיגיון מאחורי התרגול">
            <div class="screen-read-guide-illustration-media">
                <img class="screen-read-guide-philosopher" src="${escapeHtml(philosopherIconSrc || 'assets/svg/props/philosopher-guide.svg')}" alt="פילוסוף מסביר" loading="lazy">
            </div>
            <div class="screen-read-guide-illustration-copy">
                <strong>פילוסוף מסך · מה נפתח כאן באמת</strong>
                <span>${escapeHtml(philosopherTeaser)}</span>
            </div>
        </summary>
...
async function loadMetaModelData() {
    try {
        ...
        if (patternsResponse && patternsResponse.ok) {
...
function getBreenReferenceRows() {
    ...
    return [
        { id: 'row1', categories: ['mind_reading', 'assumptions', 'lost_performative'] },
        { id: 'row2', categories: ['cause_effect', 'modal_operator', 'universal_quantifier'] },
        { id: 'row3', categories: ['complex_equivalence', 'identity_predicates', 'nominalisations'] },
        { id: 'row4', categories: ['lack_referential_index', 'time_space_predicates', 'comparative_deletion'] },
        { id: 'row5', categories: ['unspecified_verbs', 'sensory_predicates', 'non_referring_nouns'] }
    ];
}
...
function getBreenReferenceCategoryLabelHe(categoryId = '') {
    ...
    const labels = {
        ...
        identity_predicates: 'פרדיקטים של זהות',
        complex_equivalence: 'הקבלה מורכבת',
        comparative_deletion: 'השמטה השוואתית',
        time_space_predicates: 'פרדיקטים של זמן ומקום',
...
function uniqueTrimmedList(items = [], limit = 10) {
    ...
    const seen = new Set();
    (items || []).forEach((entry) => {
        const value = normalizeUiText(String(entry || '').trim());
        if (!value) return;
...
function extractPatternLibraryTextList(items = [], limit = 12) {
    const source = Array.isArray(items) ? items : [];
    const normalized = source
        .map((entry) => (typeof entry === 'string' ? entry : entry?.text))
        .map((entry) => normalizeUiText(String(entry || '').trim()))
        .filter(Boolean);
    return uniqueTrimmedList(normalized, limit);
}

function buildPatternLibraryConcept(pattern = {}) {
    const patternId = normalizeGlossaryConceptId(pattern?.id || pattern?.name || '');
    ...
    return normalizeGlossaryConcept({
        ...
        meta_model_questions: extractPatternLibraryTextList(pattern?.goodQuestions || [], 18)
    });
}
...
function sortGlossaryConceptsByRequiredOrder(concepts = []) {
    return [...(Array.isArray(concepts) ? concepts : [])].sort((a, b) => {
        const rankDiff = getGlossaryRequiredOrderRank(a) - getGlossaryRequiredOrderRank(b);
        if (rankDiff !== 0) return rankDiff;
        return String(a?.he_title || '').localeCompare(String(b?.he_title || ''), 'he');
    });
}
...
function collectGlossaryConcepts() {
    ...
    (metaModelPatternLibraryData?.patterns || []).forEach((pattern) => {
        const concept = buildPatternLibraryConcept(pattern);
        upsert(concept, { preferred: false });
    });
...
function buildGlossaryConceptCard(item = {}, { lazyBody = false } = {}) {
    ...
    [
        { text: groupHe, className: 'glossary-pill glossary-pill-he' },
        { text: groupEn, className: 'glossary-pill glossary-pill-en' },
        { text: mainCategory, className: 'glossary-pill glossary-pill-en' }
    ].forEach((pill) => {
        const text = normalizeUiText(String(pill?.text || '').trim());
        if (!text) return;
        const key = text.toUpperCase();
        if (seenPills.has(key)) return;
        seenPills.add(key);
        pills.push(`<span class="${pill.className}">${escapeHtml(text)}</span>`);
    });
...
const QUESTION_DRILL_PACK = [
    ...
    Object.freeze({
        ...
        difficulty: 'easy',
        explanation: 'הקבלה מורכבת: פעולה אחת מתורגמת למשמעות רחבה.'
    }),
    Object.freeze({
        id: 'mm_easy_005',
    ...
    Object.freeze({
        ...
        difficulty: 'easy',
        explanation: 'השמטה השוואתית: יותר טוב ביחס למה.'
    }),
    Object.freeze({
        id: 'mm_medium_001',
...
const QUESTION_DRILL_TIMER_LIGHTS_MS = Object.freeze({
    green: 2500,
    yellow: 4800
});
...
function formatQuestionDrillTimerMs(value = 0) {
    const ms = Math.max(0, Number(value) || 0);
    return (ms / 1000).toFixed(2);
}

function getQuestionDrillTargetCategory(question = null) {
    const raw = String(
        question?.targetMacro
        || (Array.isArray(question?.focus) ? question.focus[0] : '')
        || ''
    ).trim().toUpperCase();
...
function getQuestionDrillTargetCopy(category = 'DELETION') {
...
function updateQuestionDrillTargetBar() {
    const bar = questionDrillState.elements.targetBar;
    if (!bar) return;
    const shouldShow = questionDrillState.mode !== 'test' && Boolean(questionDrillState.current) && !questionDrillState.sessionCompleted;
...
function setQuestionDrillTrafficState(state = 'green', label = '') {
    const nextState = ['green', 'yellow', 'red'].includes(String(state || '').trim().toLowerCase())
        ? String(state || '').trim().toLowerCase()
        : 'green';
    const fallbackLabels = {
        green: 'מוכן לסבב',
        yellow: 'צריך עוד דיוק',
        red: 'הסבב נסגר, עוברים הלאה'
    };
    questionDrillState.trafficState = nextState;
    if (questionDrillState.elements.traffic) {
        questionDrillState.elements.traffic.dataset.state = nextState;
    }
    if (questionDrillState.elements.trafficLabel) {
        questionDrillState.elements.trafficLabel.textContent = String(label || fallbackLabels[nextState] || '').trim();
    }
}

function refreshQuestionDrillTrafficState() {
    if (questionDrillState.sessionCompleted) {
        setQuestionDrillTrafficState('green', 'הסשן הושלם');
        return;
    }

    if (questionDrillState.mode === 'test' && questionDrillState.current && !questionDrillState.roundFinalized) {
        const elapsed = Math.max(0, Number(questionDrillState.testTimerMs) || 0);
        if (elapsed <= QUESTION_DRILL_TIMER_LIGHTS_MS.green) {
            setQuestionDrillTrafficState('green', 'חלון תגובה פתוח');
            return;
        }
        if (elapsed <= QUESTION_DRILL_TIMER_LIGHTS_MS.yellow) {
            setQuestionDrillTrafficState('yellow', 'להחליט עכשיו');
            return;
        }
        setQuestionDrillTrafficState('red', 'האדום מתקרב');
        return;
    }

    if (questionDrillState.roundFinalized) {
        if (questionDrillState.roundCorrect) {
            setQuestionDrillTrafficState('green', 'זיהוי נכון');
            return;
        }
        setQuestionDrillTrafficState('red', 'סוגרים סבב וממשיכים');
        return;
    }

    if (questionDrillState.roundChecks > 0 && !questionDrillState.roundCorrect) {
        setQuestionDrillTrafficState('yellow', 'כיוון קרוב, נסו שוב');
        return;
    }

    setQuestionDrillTrafficState('green', questionDrillState.mode === 'test' ? 'מוכן לזיהוי מהיר' : 'מוכן לסבב');
}

function updateQuestionDrillGameHud() {
    const isTestMode = questionDrillState.mode === 'test';
    const bestByDifficulty = questionDrillState.testBestScoreByDifficulty[questionDrillState.testDifficulty] || 0;
    const roundAward = questionDrillState.currentRoundAward || null;
    const roundAwardText = !isTestMode && roundAward && (roundAward.xp || roundAward.stars)
        ? `+${roundAward.xp || 0} / +${roundAward.stars || 0}⭐`
        : '';

    if (questionDrillState.elements.hudMode) {
        questionDrillState.elements.hudMode.textContent = getQuestionDrillModeLabel();
    }
    if (questionDrillState.elements.hudTimeLabel) {
        questionDrillState.elements.hudTimeLabel.textContent = 'זמן';
    }
    if (questionDrillState.elements.hudTime) {
        questionDrillState.elements.hudTime.textContent = isTestMode
            ? `${formatQuestionDrillTimerMs(questionDrillState.testTimerMs)}ש׳`
            : 'חופשי';
    }
    if (questionDrillState.elements.hudScoreLabel) {
        questionDrillState.elements.hudScoreLabel.textContent = isTestMode ? 'ניקוד' : 'כוכבים';
    }
    if (questionDrillState.elements.hudScore) {
        questionDrillState.elements.hudScore.textContent = String(isTestMode ? questionDrillState.testScore : questionDrillState.sessionStars);
    }
    if (questionDrillState.elements.hudStreakLabel) {
        questionDrillState.elements.hudStreakLabel.textContent = 'רצף';
    }
    if (questionDrillState.elements.hudStreak) {
        questionDrillState.elements.hudStreak.textContent = String(questionDrillState.streak);
    }
    if (questionDrillState.elements.hudBonusLabel) {
        questionDrillState.elements.hudBonusLabel.textContent = isTestMode ? 'שיא' : (roundAwardText ? 'בונוס' : 'XP');
    }
    if (questionDrillState.elements.hudBonus) {
        questionDrillState.elements.hudBonus.textContent = isTestMode
            ? String(bestByDifficulty)
            : (roundAwardText || String(questionDrillState.sessionXP));
    }

    updateQuestionDrillTargetBar();
    refreshQuestionDrillTrafficState();
}

function triggerQuestionDrillCelebration(kind = 'success') {
    const layer = questionDrillState.elements.fxLayer;
    if (!layer || prefersReducedMotion()) return;

    const glyphs = kind === 'finish'
        ? ['⭐', '✨', '🎆', '🎇', '⚡']
        : ['⭐', '✨', '⚡', '✦'];

    layer.innerHTML = '';
    for (let i = 0; i < (kind === 'finish' ? 18 : 10); i += 1) {
        const particle = document.createElement('span');
        particle.className = 'question-drill-fx-particle';
        particle.textContent = glyphs[i % glyphs.length];
        particle.style.left = `${10 + Math.random() * 78}%`;
        particle.style.top = `${18 + Math.random() * 44}%`;
        particle.style.setProperty('--x', `${Math.round((Math.random() - 0.5) * (kind === 'finish' ? 180 : 120))}px`);
        particle.style.setProperty('--y', `${Math.round(-40 - Math.random() * 90)}px`);
        particle.style.setProperty('--r', `${Math.round((Math.random() - 0.5) * 280)}deg`);
    ...
    if (layer.__questionDrillFxTimer) {
        clearTimeout(layer.__questionDrillFxTimer);
    }
    layer.__questionDrillFxTimer = setTimeout(() => {
        layer.classList.remove('is-active');
        layer.innerHTML = '';
...
function updateQuestionDrillLayoutState({ focusLiveSurface = false } = {}) {
    ...
    if (elements.gameHud) {
        elements.gameHud.classList.toggle('hidden', !showHud);
    }
...
function clearQuestionDrillTestAutoAdvance() {
    if (questionDrillState.testAutoAdvanceHandle) {
        ...
        questionDrillState.testAutoAdvanceHandle = null;
    }
}
...
function stopQuestionDrillTimer({ freeze = true } = {}) {
    if (questionDrillState.testTimerHandle) {
        clearInterval(questionDrillState.testTimerHandle);
        questionDrillState.testTimerHandle = null;
    }

    if (!freeze) {
        questionDrillState.testTimerMs = 0;
        questionDrillState.testRoundStartedAt = 0;
        updateQuestionDrillTimerDisplay();
        return 0;
    }

    if (questionDrillState.testRoundStartedAt > 0) {
        const now = (typeof performance !== 'undefined' && performance.now)
            ? performance.now()
            : Date.now();
        questionDrillState.testTimerMs = questionDrillEngine.clampReactionTimeMs(now - questionDrillState.testRoundStartedAt);
    }
    questionDrillState.testRoundStartedAt = 0;
    updateQuestionDrillTimerDisplay();
    return questionDrillState.testTimerMs;
}

function startQuestionDrillTimer() {
    if (questionDrillState.mode !== 'test' || !questionDrillState.current || questionDrillState.roundFinalized) {
    ...
    const now = (typeof performance !== 'undefined' && performance.now)
        ? performance.now()
        : Date.now();
    questionDrillState.testRoundStartedAt = now;
    questionDrillState.testTimerMs = 0;
    updateQuestionDrillTimerDisplay();

    questionDrillState.testTimerHandle = setInterval(() => {
        if (
        ...
        const tickNow = (typeof performance !== 'undefined' && performance.now)
            ? performance.now()
            : Date.now();
        questionDrillState.testTimerMs = questionDrillEngine.clampReactionTimeMs(tickNow - questionDrillState.testRoundStartedAt);
        updateQuestionDrillTimerDisplay();
    }, 70);
}

function scheduleQuestionDrillTestAutoAdvance(delayMs = QUESTION_DRILL_TEST_AUTO_ADVANCE_MS) {
    clearQuestionDrillTestAutoAdvance();
    if (questionDrillState.mode !== 'test' || questionDrillState.sessionCompleted) return;

    questionDrillState.testAutoAdvanceHandle = setTimeout(() => {
        questionDrillState.testAutoAdvanceHandle = null;
        if (questionDrillState.mode !== 'test' || questionDrillState.sessionCompleted) return;
        loadNextQuestionDrill({ playSound: false });
    }, Math.max(250, Math.floor(Number(delayMs) || QUESTION_DRILL_TEST_AUTO_ADVANCE_MS)));
}

function setQuestionDrillMode(mode = 'learning', { persist = true, refreshCurrent = false } = {}) {
    const resolved = normalizeQuestionDrillMode(mode);
    questionDrillState.mode = resolved;
    const isTestMode = resolved === 'test';
    const elements = questionDrillState.elements;

    if (elements.root) {
        elements.root.classList.toggle('is-test-mode', isTestMode);
    }
    if (elements.modeLearningBtn) {
    ...
    if (refreshCurrent && questionDrillState.current && !questionDrillState.sessionCompleted) {
        if (!questionDrillState.roundFinalized) {
            questionDrillState.options = buildQuestionDrillOptions(questionDrillState.current);
...
function setQuestionDrillDifficulty(difficulty = 'easy', { persist = true, rebuildDeck = true } = {}) {
    const nextDifficulty = normalizeQuestionDrillDifficulty(difficulty);
    questionDrillState.testDifficulty = nextDifficulty;

    if (questionDrillState.elements.difficultySelect) {
        questionDrillState.elements.difficultySelect.value = nextDifficulty;
    }
    if (rebuildDeck) {
...
function resetQuestionDrillSessionState({ preservePrefs = true } = {}) {
    ...
    questionDrillState.testTimerMs = 0;
    questionDrillState.currentRoundReactionMs = 0;
    questionDrillState.testReactionTotalMs = 0;
    questionDrillState.testReactionCount = 0;

    if (!preservePrefs) {
        questionDrillState.mode = 'learning';
        questionDrillState.planId = QUESTION_DRILL_SUCCESS_PLANS[1].id;
        questionDrillState.testDifficulty = 'easy';
    }
}
...
function renderQuestionDrillOptions() {
    ...
    questionDrillState.options.forEach((option, index) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = `question-drill-option${isTestMode ? ' question-drill-option--test' : ''}`;
...
function setQuestionDrillSelectedOption(optionId = '', { playSound = true } = {}) {
    ...
    if (questionDrillState.elements.category && questionDrillState.mode !== 'test') {
...
function revealQuestionDrillOptionResult({ selectedOptionId = '', success = false, expectedCategory = '' } = {}) {
    ...
    const expectedPrimary = QUESTION_DRILL_CATEGORIES.includes(expectedCategory) ? expectedCategory : '';
...
function updateQuestionDrillTestBanner() {
    const banner = questionDrillState.elements.testBanner;
    if (!banner) return;

    const isVisible = questionDrillState.mode === 'test' && !!questionDrillState.current;
...
function beginQuestionDrillRound(question) {
    ...
    const expectedCategory = getQuestionDrillTargetCategory(question);
    const isTestMode = questionDrillState.mode === 'test';
    const targetCopy = getQuestionDrillTargetCopy(expectedCategory);
    ...
    if (elements.category) {
        elements.category.value = expectedCategory;
    ...
    updateQuestionDrillSessionUI();
    updateQuestionDrillTargetBar();
    setQuestionDrillTrafficState('green', isTestMode ? 'חלון תגובה פתוח' : `מחפשים ${targetCopy.target}`);
    updateQuestionDrillLayoutState({ focusLiveSurface: true });
}

function buildQuestionDrillDeck() {
    const source = questionDrillState.mode === 'test'
        ? questionDrillEngine.selectItemsByDifficulty(QUESTION_DRILL_PACK, questionDrillState.testDifficulty)
        : QUESTION_DRILL_PACK.slice();
    questionDrillState.deck = shuffleArray(source.slice());
}
...
function getNextQuestionDrillItem() {
    ...
    const activePool = questionDrillState.mode === 'test'
        ? questionDrillEngine.selectItemsByDifficulty(QUESTION_DRILL_PACK, questionDrillState.testDifficulty)
        : QUESTION_DRILL_PACK;
...
function updateQuestionDrillControlsState() {
    ...
    const isTestMode = questionDrillState.mode === 'test';
    const canCheck = !isTestMode && hasQuestion && hasSelection && !sessionLocked && !questionDrillState.roundFinalized;
...
function startQuestionDrillSession({ announce = true } = {}) {
    ...
    updateQuestionDrillStats();
    updateQuestionDrillSessionUI();
    setQuestionDrillTrafficState('green', questionDrillState.mode === 'test' ? 'חלון תגובה פתוח' : 'מוכן לסבב');
    if (announce) {
        playUISound('start');
    }
...
function completeQuestionDrillSession() {
        ...
        if (isTestMode) {
            ...
            summaryEl.innerHTML = `
                <div class="question-drill-summary-head">
                    ...
                    <span>${getQuestionDrillModeLabel()} · קושי ${(QUESTION_DRILL_DIFFICULTIES.find((item) => item.id === questionDrillState.testDifficulty)?.label) || 'קל'}</span>
                </div>
                <div class="question-drill-summary-grid">
                    <div><small>ניקוד</small><strong>${questionDrillState.testScore}</strong></div>
                    <div><small>דיוק</small><strong>${accuracy}%</strong></div>
                    <div><small>רצף שיא</small><strong>${questionDrillState.bestStreak}</strong></div>
                    <div><small>זמן ממוצע</small><strong>${formatQuestionDrillTimerMs(avgMs)}ש׳</strong></div>
    ...
    triggerQuestionDrillCelebration('finish');
    setQuestionDrillTrafficState('green', 'שלב הושלם');
    updateQuestionDrillSessionUI();
    updateQuestionDrillControlsState();
}
...
function finalizeQuestionDrillRound(reason = 'next') {
    ...
    clearQuestionDrillTestAutoAdvance();
    if (questionDrillState.mode === 'test') {
        stopQuestionDrillTimer({ freeze: true });
    }
    ...
    if (isTestMode) {
        questionDrillState.currentRoundAward = { xp: 0, stars: 0, score: questionDrillState.testScore };
        if (!wasCorrect && reason === 'skip') {
            setQuestionDrillFeedback('הסבב דולג. אין ניקוד על הסבב הזה.', 'warn');
            playUISound('skip');
            setQuestionDrillTrafficState('red', 'סבב דולג');
        }
        if (wasCorrect) {
            triggerQuestionDrillCelebration('success');
            setQuestionDrillTrafficState('green', 'פגיעה מהירה');
        } else if (reason !== 'skip') {
            setQuestionDrillTrafficState('red', 'פספוס');
        }
        updateQuestionDrillSessionUI();
        updateQuestionDrillControlsState();
        if (questionDrillState.roundsDone >= plan.rounds) {
    ...
    if (wasCorrect) {
        ...
        triggerQuestionDrillCelebration(starGain >= 3 ? 'finish' : 'success');
        setQuestionDrillTrafficState('green', starGain >= 3 ? `בינגו · +${starGain}⭐` : 'זיהוי נכון');
        ...
        if (reason === 'skip') {
            ...
            playUISound('skip');
            setQuestionDrillTrafficState('red', 'דילוג');
        } else {
            setQuestionDrillTrafficState('red', 'לא נסגר נכון');
        }
    }
...
function evaluateQuestionDrill() {
    ...
    const expectedCategory = getQuestionDrillTargetCategory(questionDrillState.current);
    ...
    questionDrillState.roundCorrect = success;
    updateQuestionDrillStats();
    ...
    if (isTestMode) {
        const rtMs = stopQuestionDrillTimer({ freeze: true });
        const scoreDelta = questionDrillEngine.computeTestScoreDelta({
            correct: success,
            rtMs,
            streak: questionDrillState.streak
        });
        questionDrillState.currentRoundReactionMs = rtMs;
        questionDrillState.testScore += scoreDelta.total;
        questionDrillState.testReactionTotalMs += rtMs;
        questionDrillState.testReactionCount += 1;
        questionDrillState.testLastResults = [{
            ts: Date.now(),
            difficulty: questionDrillState.testDifficulty,
            rtMs,
            correct: success,
            target: expectedCategory,
            selected: selectedCategory
        }, ...questionDrillState.testLastResults].slice(0, 10);

        const currentBest = questionDrillState.testBestScoreByDifficulty[questionDrillState.testDifficulty] || 0;
        if (questionDrillState.testScore > currentBest) {
            questionDrillState.testBestScoreByDifficulty[questionDrillState.testDifficulty] = questionDrillState.testScore;
        }
        saveQuestionDrillTestStats();
        ...
        if (success) {
            ...
            playUISound('wrong');
            setQuestionDrillTrafficState('red', 'פספוס');
            finalizeQuestionDrillRound('test_fail');
        }
        updateQuestionDrillSessionUI();
        if (!questionDrillState.sessionCompleted) {
            scheduleQuestionDrillTestAutoAdvance(success ? 700 : 950);
        }
        return;
    }
...
function setupQuestionDrill() {
    ...
    questionDrillState.elements = {
        root: drillRoot,
        fxLayer: document.getElementById('question-drill-fx-layer'),
        prestart: document.getElementById('question-drill-prestart'),
        liveShell: document.getElementById('question-drill-live-shell'),
        targetBar: document.getElementById('question-drill-target-bar'),
        targetCopy: document.getElementById('question-drill-target-copy'),
        targetLegend: document.getElementById('question-drill-target-legend'),
        ...
        testBannerSubtitle: document.getElementById('question-drill-test-banner-subtitle'),
        ...
        gameHud: document.getElementById('question-drill-game-hud'),
        hudMode: document.getElementById('question-drill-hud-mode'),
        traffic: document.getElementById('question-drill-traffic-light'),
        trafficLabel: document.getElementById('question-drill-traffic-label'),
        hudTimeLabel: document.getElementById('question-drill-hud-time-label'),
        hudTime: document.getElementById('question-drill-hud-time'),
        hudScoreLabel: document.getElementById('question-drill-hud-score-label'),
        hudScore: document.getElementById('question-drill-hud-score'),
        hudStreakLabel: document.getElementById('question-drill-hud-streak-label'),
        hudStreak: document.getElementById('question-drill-hud-streak'),
    ...
    loadQuestionDrillTestStats();
    localizeQuestionDrillCategoryOptions();
    populateQuestionDrillPlanSelect();
    setQuestionDrillDifficulty(questionDrillState.testDifficulty, { persist: false, rebuildDeck: false });
    ...
    questionDrillState.elements.difficultySelect?.addEventListener('change', (event) => {
        setQuestionDrillDifficulty(event.target.value);
        if (questionDrillState.mode === 'test' && questionDrillState.sessionActive && !questionDrillState.sessionCompleted) {
...
const RAPID_PATTERN_BUTTONS = Object.freeze([
    ...
    Object.freeze({ id: 'assumptions', label: 'הנחות מוקדמות' }),
    Object.freeze({ id: 'lost_performative', label: 'חסרון הדובר' }),
    Object.freeze({ id: 'cause_effect', label: 'סיבה ותוצאה' }),
    Object.freeze({ id: 'modal_operator', label: 'מודל פעולה' }),
    Object.freeze({ id: 'universal_quantifier', label: 'הכללה' }),
    Object.freeze({ id: 'complex_equivalence', label: 'שקילות מורכבת' }),
    Object.freeze({ id: 'identity_predicates', label: 'פרדיקטי זהות' }),
    Object.freeze({ id: 'nominalisations', label: 'נומינליזציה' }),
    Object.freeze({ id: 'lack_referential_index', label: 'אובדן המצביע' }),
    Object.freeze({ id: 'time_space_predicates', label: 'זמן ומקום' }),
    Object.freeze({ id: 'comparative_deletion', label: 'השמטה השוואתית' }),
    Object.freeze({ id: 'unspecified_verbs', label: 'פועל לא מפורט' }),
    Object.freeze({ id: 'sensory_predicates', label: 'פרדיקטים חושיים' }),
    Object.freeze({ id: 'non_referring_nouns', label: 'שם עצם חסר' })
]);
...
function getRapidPatternCorrectCount() {
    return rapidPatternArenaState.history.filter((item) => item.isCorrect).length;
}

function getRapidPatternUiStage() {
    if (!rapidPatternArenaState.sessionStarted && !rapidPatternArenaState.sessionCompleted && !rapidPatternArenaState.currentCue) {
        return 'setup';
    }
    if (rapidPatternArenaState.roundResolved || rapidPatternArenaState.sessionCompleted) {
        return 'feedback';
    }
    return 'live';
}

function focusRapidPatternLiveSurface() {
    const target = rapidPatternArenaState.elements.liveShell
        || rapidPatternArenaState.elements.monologue
        || rapidPatternArenaState.elements.root;
    if (!target) return;
...
function updateRapidPatternSessionNote() {
    ...
    const totalRounds = RAPID_PATTERN_FEEDBACK_INTERVAL;
    if (!rapidPatternArenaState.sessionStarted && !rapidPatternArenaState.sessionCompleted) {
        noteEl.textContent = rapidPatternArenaState.mode === 'exam'
            ? 'בחרו תחום וקצב, ואז לחצו "התחל מבחן".'
            : 'בחרו מצב ותחום, ואז לחצו "התחל".';
        return;
    }

    if (rapidPatternArenaState.sessionCompleted) {
        noteEl.textContent = `הסבב הושלם. ${getRapidPatternCorrectCount()}/${Math.max(1, rapidPatternArenaState.history.length)} נכונות.`;
        return;
    }

    if (rapidPatternArenaState.roundResolved) {
        noteEl.textContent = rapidPatternArenaState.mode === 'exam'
            ? `שאלה ${rapidPatternArenaState.round}/${totalRounds} נבדקה. לחצו "הבא".`
            : `קיבלתם משוב קצר. לחצו "הבא" להמשך.`;
        return;
    }

    noteEl.textContent = `סבב של ${totalRounds} שאלות. שאלה ${Math.max(1, rapidPatternArenaState.round)}/${totalRounds}.`;
}

function clearRapidPatternRoundFeedback() {
    rapidPatternArenaState.roundResolved = false;
...
function setRapidPatternRoundFeedback({ title = '', body = '', tone = 'info' } = {}) {
    rapidPatternArenaState.roundResolved = true;
    rapidPatternArenaState.feedbackTone = tone;
    rapidPatternArenaState.feedbackTitle = title;
    rapidPatternArenaState.feedbackBody = body;
    updateRapidPatternFeedbackCard();
    updateRapidPatternLayoutState();
}

function updateRapidPatternLayoutState({ focusLiveSurface = false } = {}) {
    const stage = getRapidPatternUiStage();
    const elements = rapidPatternArenaState.elements;
    ...
    if (elements.feedbackShell) {
        elements.feedbackShell.classList.toggle('hidden', stage !== 'feedback');
    }
    if (elements.openExplanationBtn) {
        const showExplanation = Boolean(rapidPatternArenaState.currentCue) && (rapidPatternArenaState.roundResolved || rapidPatternArenaState.sessionCompleted);
        elements.openExplanationBtn.classList.toggle('hidden', !showExplanation);
    }
    if (elements.nextBtn) {
        elements.nextBtn.textContent = rapidPatternArenaState.sessionCompleted ? 'תרגול חדש' : 'הבא';
    }
    if (elements.skipBtn) {
        const showSkip = stage === 'live' && Boolean(rapidPatternArenaState.currentCue) && !rapidPatternArenaState.paused;
        elements.skipBtn.classList.toggle('hidden', !showSkip);
    }

    updateRapidPatternFeedbackCard();
    updateRapidPatternSessionNote();

    if (focusLiveSurface && stage !== 'setup') {
        window.requestAnimationFrame(() => focusRapidPatternLiveSurface());
    }
}

function openRapidPatternOverlay({ type = 'rapid-pattern', title = 'חלון', size = 'md', content = null } = {}) {
    if (!window.MetaOverlayProvider || typeof window.MetaOverlayProvider.openOverlay !== 'function') return false;
    window.MetaOverlayProvider.openOverlay({
        type,
        title,
...
function buildRapidPatternHelpOverlayContent() {
    ...
    wrapper.innerHTML = `
        ...
        <h4>איך עובדים כאן</h4>
        <ol class="shell-help-list">
            <li>קראו את המשפט ושימו לב לביטוי המודגש.</li>
            <li>בחרו את התבנית שהכי מתאימה.</li>
            <li>קבלו משוב קצר והמשיכו לשאלה הבאה.</li>
        </ol>
    `;
    ...
    if (rapidPatternArenaState.mode === 'exam' && rapidPatternArenaState.active) {
        note.textContent = 'במצב מבחן אין עזרה בזמן השאלה. אפשר לבדוק כאן את שיטת העבודה בין שאלות.';
        wrapper.appendChild(note);
        return wrapper;
    }

    if (rapidPatternArenaState.currentCue && rapidPatternArenaState.mode === 'learning') {
        const contextual = document.createElement('div');
        contextual.className = 'rapid-radar-overlay-block';
        contextual.innerHTML = buildRapidPatternHelpHtml(rapidPatternArenaState.currentCue);
        wrapper.appendChild(contextual);
        note.textContent = 'העזרה כאן ממוקדת לטריגר הנוכחי כדי שלא תצטרכו לחפש אותה בתוך המסך.';
    } else {
        note.textContent = 'בחרו מצב, תחום וקצב, ואז התחילו סבב של 10 שאלות.';
    }
...
function buildRapidPatternStatsOverlayContent() {
    const wrapper = document.createElement('article');
    wrapper.className = 'shell-overlay-content rapid-radar-overlay-content';
    const list = document.createElement('ul');
    list.className = 'shell-stats-list rapid-radar-overlay-list';
    const correctCount = getRapidPatternCorrectCount();
    const attempts = rapidPatternArenaState.history.length;
    ...
    const selectedTypeLabel = selectedType === 'random' ? 'אקראי' : selectedType;

    [
        `מצב: ${rapidPatternArenaState.mode === 'exam' ? 'מבחן' : 'למידה'}`,
        `תחום: ${selectedTypeLabel}`,
        `זמן לשאלה: ${rapidPatternArenaState.timeLimitSec} שנ׳`,
        `שאלה נוכחית: ${rapidPatternArenaState.round}/${RAPID_PATTERN_FEEDBACK_INTERVAL}`,
        `נכונות: ${correctCount}/${attempts || 0}`,
        `ניקוד: ${rapidPatternArenaState.score}`,
        `רצף: ${rapidPatternArenaState.streak}`,
        `דיוק: ${accuracy}%`
    ].forEach((line) => {
        const item = document.createElement('li');
        item.textContent = line;
    ...
    wrapper.innerHTML = `
        <p class="shell-overlay-kicker">נתונים</p>
        <h4>${rapidPatternArenaState.sessionCompleted ? 'סיכום הסבב' : 'מצב נוכחי'}</h4>
    `;
    wrapper.appendChild(list);

    if (rapidPatternArenaState.sessionCompleted && rapidPatternArenaState.history.length) {
        const summary = document.createElement('div');
        summary.className = 'rapid-radar-overlay-block';
        summary.innerHTML = buildRapidPatternAiFeedbackHtml(
            rapidPatternArenaState.history.slice(-RAPID_PATTERN_FEEDBACK_INTERVAL)
        );
    ...
    const note = document.createElement('p');
    note.className = 'shell-overlay-note';
    note.textContent = 'הנתונים המלאים נשארים מחוץ למסך הראשי כדי שהתרגול עצמו יישאר במרכז.';
    wrapper.appendChild(note);
    return wrapper;
}

function buildRapidPatternExplanationOverlayContent() {
    const wrapper = document.createElement('article');
    wrapper.className = 'shell-overlay-content rapid-radar-overlay-content';
    ...
    if (!cue) {
        wrapper.innerHTML = `
            <p class="shell-overlay-kicker">הסבר</p>
            <h4>עדיין אין שאלה פעילה</h4>
            <p class="shell-overlay-note">התחילו סבב כדי לקבל הסבר ממוקד על המשפט הנוכחי.</p>
        `;
        return wrapper;
    }

    const label = getRapidPatternLabel(normalizeRapidPatternId(cue.patternId));
    const meta = RAPID_PATTERN_BUTTONS.find((item) => item.id === normalizeRapidPatternId(cue.patternId));
    const quote = document.createElement('blockquote');
    quote.className = 'rapid-radar-overlay-quote';
    quote.textContent = cue.monologue || '';

    wrapper.innerHTML = `
        <p class="shell-overlay-kicker">הסבר</p>
        <h4>${escapeHtml(label)}</h4>
        <p>הטריגר כאן הוא "${escapeHtml(cue.highlight || 'הביטוי המודגש')}", והוא מכוון לתבנית הזאת.</p>
        <p class="shell-overlay-note">${escapeHtml(meta?.hint || 'שימו לב לאופן שבו המשפט מציג הנחה, הכללה או קשר אוטומטי.')}</p>
    `;
    wrapper.appendChild(quote);
    return wrapper;
}

function openRapidPatternHelpOverlay() {
    openRapidPatternOverlay({
        type: 'rapid-pattern-help',
        title: 'עזרה',
        size: 'md',
        content: buildRapidPatternHelpOverlayContent()
    });
}

function openRapidPatternStatsOverlay() {
    openRapidPatternOverlay({
        type: 'rapid-pattern-stats',
        title: 'נתונים',
        size: 'md',
        content: buildRapidPatternStatsOverlayContent()
    });
}

function openRapidPatternExplanationOverlay() {
    openRapidPatternOverlay({
        type: 'rapid-pattern-explanation',
        title: 'למה זו התשובה',
        size: 'md',
        content: buildRapidPatternExplanationOverlayContent()
    });
}

function openRapidPatternMenuOverlay() {
    const wrapper = document.createElement('article');
    wrapper.className = 'shell-overlay-content rapid-radar-overlay-content';
    wrapper.innerHTML = `
        <p class="shell-overlay-kicker">כלים</p>
        <h4>פעולות משניות</h4>
        <p class="shell-overlay-note">פותחים מכאן רק מה שצריך, בלי להעמיס על משטח התרגול.</p>
    `;

    const actions = document.createElement('div');
    actions.className = 'shell-overlay-actions rapid-radar-overlay-actions';
    ...
    if (rapidPatternArenaState.mode !== 'exam' && rapidPatternArenaState.sessionStarted && !rapidPatternArenaState.sessionCompleted) {
        addAction(rapidPatternArenaState.paused ? 'המשך' : 'עצור', 'btn btn-secondary', () => {
            toggleRapidPatternPause();
            window.MetaOverlayProvider?.closeOverlay('rapid-pattern-menu');
        });
    }
    ...
    addAction(
        rapidPatternArenaState.sessionCompleted ? 'תרגול חדש' : 'התחל מחדש',
        'btn btn-primary',
        () => {
            window.MetaOverlayProvider?.closeOverlay('rapid-pattern-menu');
            resetRapidPatternToSetup({ preservePrefs: true });
        }
    );

    wrapper.appendChild(actions);
    openRapidPatternOverlay({
        type: 'rapid-pattern-menu',
        title: 'כלים',
        size: 'sm',
        content: wrapper
    });
...
function setupRapidPatternArena() {
    ...
    rapidPatternArenaState.elements = {
        root,
        prestart: document.getElementById('rapid-radar-prestart'),
        liveShell: document.getElementById('rapid-radar-live-shell'),
        sessionNote: document.getElementById('rapid-session-note'),
        ...
        openHelpBtn: document.getElementById('rapid-open-help-prestart'),
        openStatsBtn: document.getElementById('rapid-open-stats-prestart'),
        openMenuBtn: document.getElementById('rapid-open-menu'),
        compactRound: document.getElementById('rapid-compact-round'),
        compactCorrect: document.getElementById('rapid-compact-correct'),
        compactStreak: document.getElementById('rapid-compact-streak'),
        feedbackShell: document.getElementById('rapid-radar-feedback-shell'),
        resultCard: document.getElementById('rapid-result-card'),
        feedbackTitle: document.getElementById('rapid-feedback-title'),
        feedbackBody: document.getElementById('rapid-feedback-body'),
        openExplanationBtn: document.getElementById('rapid-open-explanation'),
        nextBtn: document.getElementById('rapid-next-btn'),
        skipBtn: document.getElementById('rapid-skip-btn'),
    ...
    rapidPatternArenaState.sessionStarted = false;
    rapidPatternArenaState.sessionCompleted = false;
    rapidPatternArenaState.roundResolved = false;
    rapidPatternArenaState.feedbackTone = 'info';
    rapidPatternArenaState.feedbackTitle = '';
    rapidPatternArenaState.feedbackBody = '';
    ...
    setRapidPatternFeedback('בחרו מצב ותחום, ואז לחצו "התחל".', 'info');
    ...
    hideRapidPatternExplanation({ resumeIfNeeded: false });
    setRapidPatternFocusMode(false);
    clearRapidPatternRoundFeedback();
    ...
    rapidPatternArenaState.elements.openHelpBtn?.addEventListener('click', openRapidPatternHelpOverlay);
    rapidPatternArenaState.elements.openStatsBtn?.addEventListener('click', openRapidPatternStatsOverlay);
    rapidPatternArenaState.elements.openMenuBtn?.addEventListener('click', openRapidPatternMenuOverlay);
    rapidPatternArenaState.elements.openExplanationBtn?.addEventListener('click', openRapidPatternExplanationOverlay);
    rapidPatternArenaState.elements.nextBtn?.addEventListener('click', handleRapidPatternNextAction);
    rapidPatternArenaState.elements.skipBtn?.addEventListener('click', skipRapidPatternCue);
...
function setRapidPatternMode(mode = 'learning', { persist = true, announce = true } = {}) {
    ...
    if (modeNote) {
        modeNote.textContent = isLearning
            ? 'למידה: אפשר לעצור ולקבל עזרה.'
            : 'מבחן: רצף מהיר בלי עצירה.';
    }
    if (root) {
        root.dataset.rapidMode = resolvedMode;
    }
    if (rapidPatternArenaState.elements.startBtn && !rapidPatternArenaState.sessionStarted && !rapidPatternArenaState.sessionCompleted) {
        rapidPatternArenaState.elements.startBtn.textContent = isLearning ? 'התחל' : 'התחל מבחן';
    }
    ...
    if (isLearning) {
        ...
        updateRapidPatternExplainButtonState();
        if (announce) {
            setRapidPatternFeedback('מצב לימוד פעיל: אפשר לעצור ולקבל עזרה.', 'info');
        }
        updateRapidPatternScoreboard();
        updateRapidPatternSessionNote();
        return;
    ...
    updateRapidPatternExplainButtonState();
    if (announce) {
        setRapidPatternFeedback('מצב מבחן פעיל: עובדים ברצף בלי עצירה.', 'info');
    }
    updateRapidPatternScoreboard();
    updateRapidPatternSessionNote();
}
...
function buildRapidPatternHelpHtml(cue) {
    ...
    const hint = buttonMeta?.label || 'בדקו איזו הנחה לשונית נחשפת במילה המודגשת.';
...
function showRapidPatternHelp() {
    ...
    if (!rapidPatternArenaState.currentCue) {
        setRapidPatternFeedback('התחילו סבב ואז פתחו עזרה ממוקדת לשאלה הפעילה.', 'warn');
        return;
    }
...
function populateRapidPatternTypes() {
    ...
    select.innerHTML = '<option value="random">אקראי</option>';
...
function resetRapidPatternToSetup({ preservePrefs = true } = {}) {
    ...
    rapidPatternArenaState.feedbackBody = '';
    if (!preservePrefs) {
        setRapidPatternMode('learning');
    }

    if (rapidPatternArenaState.elements.startBtn) {
        rapidPatternArenaState.elements.startBtn.textContent = rapidPatternArenaState.mode === 'exam' ? 'התחל מבחן' : 'התחל';
    }
    setRapidPatternTrafficLight('green');
    updateRapidPatternScoreboard();
    setRapidPatternButtonsDisabled(true);
    ...
    setRapidPatternHelpButtonState(false);
    updateRapidPatternExplainButtonState();
    updateRapidPatternTimerVisual(rapidPatternArenaState.timeLimitSec * 1000);
    setRapidPatternFeedback('בחרו מצב ותחום, ואז לחצו "התחל".', 'info');
    renderRapidPatternIdlePreview();
    updateRapidPatternLayoutState();
}
...
function startRapidPatternSession() {
    ...
    rapidPatternArenaState.paused = false;
    rapidPatternArenaState.sessionStarted = true;
    rapidPatternArenaState.sessionCompleted = false;
    rapidPatternArenaState.roundResolved = false;
    rapidPatternArenaState.pauseReason = '';
    rapidPatternArenaState.pendingNextCue = false;
...
function setRapidPatternPauseButtonState(enabled, paused = false) {
    ...
    btn.textContent = paused ? 'המשך' : 'עצור';
}
...
function pauseRapidPatternSession(reason = 'manual') {
    ...
    clearRapidPatternNextTimer();
    setRapidPatternButtonsDisabled(true);
    setRapidPatternFeedback('הסבב בהשהיה. כשתרצו, לחצו על "המשך".', 'warn');
    if (reason === 'help') {
        setRapidPatternFeedback('העזרה פתוחה. המשיכו כשתהיו מוכנים.', 'warn');
    }
    if (reason === 'explain') {
        setRapidPatternFeedback('ההסבר פתוח. חזרו לתרגול כשתסגרו אותו.', 'warn');
    }
...
function handleRapidPatternNextAction() {
    ...
    if (!rapidPatternArenaState.roundResolved) return;
    moveToNextRapidPatternCue();
}
...
function skipRapidPatternCue() {
    ...
    updateRapidPatternScoreboard();
    setRapidPatternFeedback(`דילוג. התשובה: ${getRapidPatternLabel(normalizeRapidPatternId(cue.patternId))}.`, 'warn');
    playUISound('select_soft');
...
function finishRapidPatternRoundIfNeeded() {
    ...
    rapidPatternArenaState.paused = false;
    rapidPatternArenaState.sessionStarted = true;
    rapidPatternArenaState.sessionCompleted = true;
    rapidPatternArenaState.roundResolved = true;
    rapidPatternArenaState.pauseReason = '';
    rapidPatternArenaState.pendingNextCue = false;
    rapidPatternArenaState.pausedRemainingMs = 0;
...
function buildRapidPatternAiFeedbackHtml(history) {
    ...
    return `
        ...
        <p class="rapid-ai-summary">להמשך: פתחו תרגול חדש לעוד בלוק של ${RAPID_PATTERN_FEEDBACK_INTERVAL} שאלות.</p>
    `;
}
...
function updateRapidPatternScoreboard() {
    ...
    if (compactRoundEl) {
        const visibleRound = rapidPatternArenaState.round > 0 ? rapidPatternArenaState.round : 0;
        compactRoundEl.textContent = `שאלה ${visibleRound}/${RAPID_PATTERN_FEEDBACK_INTERVAL}`;
    }
    if (compactCorrectEl) {
    ...
    if (compactStreakEl) {
        compactStreakEl.textContent = rapidPatternArenaState.mode === 'exam'
            ? `זמן ${rapidPatternArenaState.timeLimitSec}ש׳`
            : `רצף ${rapidPatternArenaState.streak}`;
    }
}
...
function setupScenarioTrainerModule() {
    ...
    scenarioTab.setAttribute('data-scenario-host-mode', launcherLink ? 'launcher' : 'launcher-missing');
...
function renderPrismScoreInterpretation(score, mismatchCount) {
    ...
    if (score.total >= 85) {
        ...
        notes.push('המיפוי חלקי. לפני צעד הבא עמוק, מומלץ לסדר את השיבוצים ולדייק ניסוחים.');
    } else {
...
function renderPrismLevelsDeepAnalysis(prism, recommendation) {
    ...
    const items = order.map(level => {
        ...
        const pivotTag = recommendation.pivot === level
            ? '<p><strong>סטטוס:</strong> זו רמת ה-צעד הבא המומלצת כרגע.</p>'
            : '';
...
function renderPrismActionPlan(session, recommendation, mismatchCount) {
    ...
    const resistanceStep = highResistance
        ? 'עבודה עם התנגדות גבוהה: התחל ב-Small Win חיצוני (סביבה/התנהגות) לפני שינוי אמונות עמוק.'
        : 'אפשר להתקדם לעומק: אחרי ביצוע צעד קטן, עבור לעבודה ברמות יכולות/ערכים/זהות.';
    ...
    return `
        <ol class="prism-action-plan">
            ...
            <li><strong>צעד 2 (צעד הבא מעשי):</strong> בצע פעולה אחת לפי רמת ${recommendation.levelName}: ${recommendation.intervention}</li>
...
function renderPrismResult(session, recommendation) {
    ...
    out.innerHTML = `
        ...
        <div class="blueprint-section prism-pivot-box">
            <h4>המלצת צעד הבא - הסבר מעמיק</h4>
            <p><strong>צעד הבא מומלץ:</strong> ${recommendation.levelName}</p>
            ...
            <ul>${countsHtml}</ul>
            <p><strong>משמעות מעשית:</strong> ה-צעד הבא הוא נקודת המינוף הכי יעילה כרגע. מיקוד נכון ברמה הזו יוצר תזוזה מהירה ואז מאפשר עבודה עמוקה יותר.</p>
        </div>
    ...
    out.innerHTML = `
        ...
        <div class="blueprint-section prism-focus-box">
            ...
            <ul class="prism-action-plan">
                ${focusItems || '<li>נראה טוב. אפשר לעבור לביצוע ה-צעד הבא שנבחר.</li>'}
...
function ceflowNormChoice(raw, fallbackId) {
    ...
    const natural = CEFLOW_NATURAL_CHOICE_COPY[id] || {};
    const replyOptions = Array.isArray(raw?.replyOptions)
        ? raw.replyOptions.map(v => ceflowNormalizeCopy(v, '')).filter(Boolean).slice(0, 4)
        : [];
    const fallbackReplyOptions = Array.isArray(natural.replyOptions)
        ? natural.replyOptions.map(v => ceflowNormalizeCopy(v, '')).filter(Boolean).slice(0, 4)
        : [];
    const repairOptions = Array.isArray(raw?.repairOptions)
        ? raw.repairOptions.map(v => ceflowNormalizeCopy(v, '')).filter(Boolean).slice(0, 4)
        : [];
    const fallbackRepairOptions = Array.isArray(natural.repairOptions)
        ? natural.repairOptions.map(v => ceflowNormalizeCopy(v, '')).filter(Boolean).slice(0, 4)
        : [];
    return {
        id,
        label: ceflowNormalizeCopy(raw?.label, natural.label || CEFLOW_DEFAULT_LABELS[id] || f.label || 'תגובה'),
        tone: ceflowNormalizeCopy(raw?.tone, f.tone || 'muted'),
        emoji: ceflowNormalizeCopy(raw?.emoji, f.emoji || '💬'),
        say: ceflowNormalizeCopy(raw?.say, natural.say || ''),
        counterReply: ceflowNormalizeCopy(raw?.counterReply, natural.counterReply || f.counterReply || ''),
        replyPrompt: ceflowNormalizeCopy(raw?.replyPrompt, 'איך את/ה עונה עכשיו?'),
        replyOptions: replyOptions.length ? replyOptions : (fallbackReplyOptions.length ? fallbackReplyOptions : [
            'בוא/י ננשום רגע ונגדיר צעד ראשון.',
            'איפה בדיוק נתקעת?',
            'מה כבר כן עובד, אפילו חלקית?'
        ]),
        repairOptions: id === 'meta'
            ? []
            : (repairOptions.length ? repairOptions : (fallbackRepairOptions.length ? fallbackRepairOptions : [...CEFLOW_DEFAULT_REPAIR_OPTIONS])),
        ...
        powerQuestions: Array.isArray(raw?.powerQuestions)
            ...
            : [],
        newInfoBubble: ceflowNormalizeCopy(raw?.newInfoBubble, ''),
        blueprint: ceflowNormBlueprint(raw?.blueprint)
    };
}

function ceflowNormContext(raw) {
    ...
    return {
        ...
        distractors: distractors.length
            ? distractors
            : [
                'הטלפון מצפצף: הודעה דחופה.',
                'מישהו קורא לך מהחדר השני.',
                'עוד משימה קופצת בדיוק באמצע השיחה.'
            ],
        shotClockSeconds: ceflowClampInt(
            source?.shotClockSeconds || source?.shot_clock || source?.timerSeconds,
            CEFLOW_DEFAULT_SHOT_CLOCK_SECONDS,
            CEFLOW_MIN_SHOT_CLOCK_SECONDS,
...
function ceflowNormScenario(raw, i) {
    ...
    if (meta && !meta.powerQuestions.length) {
        meta.powerQuestions = [
            'מה בדיוק לא ברור כרגע?',
            'איזה צעד ראשון הכי קטן כן אפשרי?',
            'איזה מידע חסר כדי להתקדם?',
            'מה אפשר להוריד מהעומס עכשיו?',
            'מה מגדיר הצלחה בדקה הקרובה?'
        ];
    }
    if (meta && !meta.newInfoBubble) {
        meta.newInfoBubble = 'עכשיו זה ברור יותר: אפשר להתחיל מצעד קטן במקום להיתקע על הכול.';
    }
    const context = ceflowNormContext(raw);
    return {
        id: ceflowNormalizeCopy(raw.id, `scene_${i + 1}`),
        domain: ceflowNormalizeCopy(raw.domain, 'כללי'),
        title: ceflowNormalizeCopy(raw.title, `סצנה ${i + 1}`),
        level: ceflowNormalizeCopy(raw.level || raw.levelTag, 'מודליות + הכללה'),
        regulationNote: ceflowNormalizeCopy(
            raw.regulationNote,
            'לכולנו יש תגובה רגשית אימפולסיבית. התרגול כאן הוא לזהות אותה, לווסת סטייט, ולעבור לתגובה שכלית מבוססת שאלה.'
        ),
        context,
        shotClockSeconds: context.shotClockSeconds,
        characters: {
            left: {
                name: ceflowNormalizeCopy(raw?.characters?.left?.name, 'דמות שמאל'),
                sprite: ceflowNormalizeCopy(raw?.characters?.left?.sprite, '')
            },
            right: {
                name: ceflowNormalizeCopy(raw?.characters?.right?.name, 'דמות ימין'),
                sprite: ceflowNormalizeCopy(raw?.characters?.right?.sprite, '')
            }
        },
        dialog: (raw.dialog || []).map(item => ({
            speaker: item?.speaker === 'right' ? 'right' : 'left',
            text: ceflowNormalizeCopy(item?.text, ''),
...
function ceflowBuildScenarioPool(rawScenarios) {
    const external = (Array.isArray(rawScenarios) ? rawScenarios : [])
        .map((scenario, i) => ceflowNormScenario(scenario, i))
        .filter(Boolean);
    const injected = CEFLOW_PRESSURE_SCENARIOS
        .map((scenario, i) => ceflowNormScenario(scenario, i))
        .filter(Boolean);

    const byId = new Map();
    injected.forEach(scene => {
        if (!scene?.id) return;
        byId.set(scene.id, scene);
    });
    external.forEach(scene => {
        if (!scene?.id) return;
...
function renderComicEngineLoadError(els, message = 'אירעה תקלה בטעינת במת הקומיקס הרגשי. נסו שוב.') {
    ...
    els.timeout?.classList.add('hidden');
    ...
    Promise.resolve(setupComicEngine2({ force }))
        .catch((error) => {
            ...
            const els = {
                ...
                timeout: document.getElementById('ceflow-timeout'),
...
async function setupComicEngine2({ force = false } = {}) {
    ...
    const scenarios = ceflowBuildScenarioPool(payload?.scenarios || []);
    if (!scenarios.length) {
        root.dataset.ceflowBootState = 'error';
    ...
    const pressureAudio = (() => {
        ...
        const ensureCtx = (forceResume) => {
            ...
            if (forceResume && ctx.state === 'suspended') {
                try {
        ...
        const playTone = (options) => {
            const audioCtx = ensureCtx(true);
            if (!audioCtx || audioCtx.state !== 'running') return;
            const now = audioCtx.currentTime + Number(options?.delay || 0);
            const osc = audioCtx.createOscillator();
            const gain = audioCtx.createGain();
            osc.type = options?.type || 'sine';
            osc.frequency.setValueAtTime(Math.max(60, Number(options?.from || 440)), now);
            if (Number.isFinite(options?.to) && Number(options.to) > 0 && Number(options.to) !== Number(options.from)) {
                osc.frequency.exponentialRampToValueAtTime(Math.max(50, Number(options.to)), now + Math.max(0.02, Number(options?.dur || 0.1)));
            }
            const peak = Math.max(0.0002, Number(options?.amp || 0.02));
            const dur = Math.max(0.04, Number(options?.dur || 0.08));
            gain.gain.setValueAtTime(0.0001, now);
        ...
        return {
            ...
            buzzer() {
                ...
                lastBuzzerAt = now;
                playTone({ type: 'sawtooth', from: 220, to: 170, dur: 0.2, amp: 0.036 });
                playTone({ type: 'square', from: 180, to: 140, dur: 0.2, delay: 0.08, amp: 0.03 });
            }
        };
    })();
    ...
    const hideDistractor = () => {
        if (state.distractorTimer) {
            clearTimeout(state.distractorTimer);
            state.distractorTimer = null;
        }
        if (els.distractor) {
            els.distractor.classList.add('hidden');
            els.distractor.textContent = '';
        }
        state.activeDistractor = '';
    };
    const showDistractor = (text) => {
        if (!els.distractor) return;
        const msg = compactText(text, 130);
        if (!msg) return;
        hideDistractor();
        state.activeDistractor = msg;
        els.distractor.textContent = `⚠️ אירוע נוסף: ${msg}`;
        els.distractor.classList.remove('hidden');
        els.root.classList.add('ceflow-pressure-pulse');
        setTimeout(() => els.root.classList.remove('ceflow-pressure-pulse'), 260);
        if (typeof navigator !== 'undefined' && typeof navigator.vibrate === 'function') {
            try { navigator.vibrate(30); } catch (_error) {}
        }
        state.distractorTimer = window.setTimeout(() => {
            if (els.distractor) els.distractor.classList.add('hidden');
        }, 3200);
    };
    ...
    const roundDurationSeconds = () => ceflowClampInt(
        currentScenario()?.shotClockSeconds || currentScenario()?.context?.shotClockSeconds,
        CEFLOW_DEFAULT_SHOT_CLOCK_SECONDS,
        CEFLOW_MIN_SHOT_CLOCK_SECONDS,
        CEFLOW_MAX_SHOT_CLOCK_SECONDS
    );
    ...
    const currentShotClockState = () => {
        ...
        let remainingMs = state.shotClockDeadlineMs > 0
            ? Math.max(0, state.shotClockDeadlineMs - Date.now())
            : totalMs;
    ...
    const stopShotClock = () => {
        if (state.shotClockTimer) {
            clearInterval(state.shotClockTimer);
            state.shotClockTimer = 0;
        }
        state.shotClockTotalMs = 0;
    ...
    const tickShotClockLoop = () => {
        if (!isEngineVisible() || state.flowState !== CEFLOW_STATES.SCENE_READY) {
            stopShotClock();
            renderShotClock();
            return;
        }
        const now = Date.now();
        const remainingMs = Math.max(0, state.shotClockDeadlineMs - now);
        if (remainingMs <= 0) {
            renderShotClock();
            timeoutRound();
            return;
        ...
        const tickInterval = remainingMs <= 5000 ? 500 : 1000;
        ...
        if (!state.shotClockDistractorFired && elapsedRatio >= 0.4) {
            state.shotClockDistractorFired = true;
            const distractors = currentScenario()?.context?.distractors || [];
            const randomText = distractors.length
    ...
    const startShotClock = () => {
        if (!isEngineVisible()) return;
        if (state.flowState !== CEFLOW_STATES.SCENE_READY) return;
        if (state.shotClockTimer) return;
        const seconds = roundDurationSeconds();
        state.shotClockTotalMs = Math.max(1000, seconds * 1000);
    ...
    const syncShotClockLifecycle = () => {
        if (state.flowState === CEFLOW_STATES.SCENE_READY) startShotClock();
        else stopShotClock();
        renderShotClock();
    };
    ...
    const renderDialog = () => {
        ...
        if (state.activeDistractor && state.flowState === CEFLOW_STATES.SCENE_READY) {
            lines.push({ speaker: 'left', text: `אירוע נוסף: ${state.activeDistractor}`, role: 'new-info' });
        }
    ...
    const renderControls = () => {
        const hasRoundOutcome = !!state.selectedChoice || !!state.userReply || state.flowState === CEFLOW_STATES.TIMEOUT;
        if (els.retry) els.retry.disabled = !hasRoundOutcome;
    ...
    document.addEventListener('keydown', () => pressureAudio.arm(), { once: true, capture: true });
    document.addEventListener('touchstart', () => pressureAudio.arm(), { once: true, capture: true, passive: true });
