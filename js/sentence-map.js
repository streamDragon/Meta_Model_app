...
const MODE_COPY = Object.freeze({
    ...
    function setupSentenceMap() {
        ...
        function renderCurrentStep(caseData, caseUi) {
            ...
            if (caseUi.stepIndex === 1) return `<section class="sentence-map-stage-card">${renderStageHeader(1, 'המשפט כמו שהוא', 'משאירים את המשפט מול העיניים ועובדים ממנו.')}${renderSentenceBubble(caseData.sentence, caseData.title)}</section>`;
            if (caseUi.stepIndex === 2) return `<section class="sentence-map-stage-card">${renderStageHeader(2, 'פותחים שכבה אחרי שכבה', 'פתחו כל כרטיס וראו את השאלה ואת הדוגמה.')}${renderSentenceBubble(caseData.sentence, caseData.title)}<div class="sentence-map-layer-stack">${LAYER_ORDER.map((layerId) => renderLayerExplorerCard(layerId, caseData, caseUi)).join('')}</div></section>`;
